...
(define (typeof tc e)
  (type-case E e
    ...
    [Pair-case (e x1 x2 eBody)
               (type-case Type (typeof tc e)
                          [T* (A1 A2)
                              (let* ([tc-extended (tc/cons-tp x1 A1 (tc/cons-tp x2 A2 tc))]
                                     [B (typeof tc-extended eBody)])
                                B)]
                          [else #false])]
    ...
    [Branch (eKey eL eR) (let* ([AKey (typeof tc eKey)]
                                [AL (typeof tc eL)]
                                [AR (typeof tc eR)])
                           (if (and (Ttree? AL)
                                     (Ttree? AR)
...
(test (typeof-program (parse '{Pair 1 2})) (T* (Tnum) (Tnum)))
